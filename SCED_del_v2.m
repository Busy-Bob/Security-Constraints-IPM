function ret = SCED_del_v2(mpc, options, ratio)
%%%%
% @para
% - mpc: matpower case
...
nums = ceil(ratio * size(branch_idx, 1));
gap = min(min(branch_max - Bf(branch_idx, :) * value(theta), Bf(branch_idx, :) * value(theta) + branch_max), [], 2);
[tmp, idx] = sort(gap, 'ascend');
branch_idx = branch_idx(idx(1:nums));
branch_max = branch_max(idx(1:nums), :);
